...
Либо просто запускаем интересующие ячейки в этом ноутбуку на kaggle:
TODO

(но, например, обучение REINFORCE может потребовать весов от тренировки reward model)

Либо заходим на kaggle, создаем ноутбук с доступом в интернет. Устанавливаем `trl`:
...
Теперь можно запускать необходимые скрипты. Примеры можно посмотреть в TODO
...
получаем `reinforce-2-expected-v3`. Первые ~75 батчей он падает, что, скорее всего из-за случайности, а потом растет, что логично потому что `v2` падало. Но оно растет гораздо сильнее, чем `v2` падает (например advantage в конце по модулю почти всегда >0.5, а у `v2` он держался около ноля).

Такое объяснить не очень могу потому что основной алгоритм REINFORCE w/ baseline не менялся и раньше он повышал награду, а теперь просто поменялась функция, которая вычисляет награду, но самому алгоритму должно быть все равно какой вид имеет эта функций так что он должен был повышать награду (возможно с меньшей скоростью потому что новая reward model и у них масштаб наград разный), но он ее понижает.
...
$$\boxed{ \mathcal{L}_{RM}^{(i)} = \frac{\text{previous loss}^{(i)}}{\sqrt{\mathbb{D}(\text{reward}^{(i)})}} }$$
